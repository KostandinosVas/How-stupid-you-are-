...
export default function TestPage() {
  ...
  }, [currentIndex, session, router]);

  const { timeLeft, start, pause, reset } = useTimer(timeLimit, handleNext);

  // When the question changes, decide whether to enter memorize phase
  useEffect(() => {
  ...
  // Memorize phase countdown — auto-advance when it hits 0
  useEffect(() => {
  ...
  // ── Phase 2 (or non-memory): question + options ──
  return (
    <MainLayout title={currentQuestion.dimensionName} progress={progress}>
